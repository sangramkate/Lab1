...
void cudaScan(int* inarray, int length, int* resultarray)
{
    ...
    int* device_out;
    cudaMalloc(&device_out,rounded_length * sizeof(int));
    cudaMemset(device_out,0,rounded_length * sizeof(int));
    exclusive_scan(inarray, length, device_out);
    printf("exclusive scan done\n");
    // Wait for any work left over to be completed.
    cudaDeviceSynchronize();
    process_repeat_kernel<<<(rounded_length + threadsPerBlock - 1)/threadsPerBlock, threadsPerBlock>>>(rounded_length, resultarray, device_out);
    printf("process repeats done\n");
    cudaDeviceSynchronize();
}
...
int fillPairs ( int* predicateArray, int length, int* outputArray){
  ...
  cudaScan(predicateArray,length,buffer);
...
CudaRenderer::circleInBoxTest(int numCircles, int numBlocks, int blocksPerRow){
 ...
 //  cudaMalloc(&circlePerBlock,sizeof(int)*(numBlocks+1));
   ...
   sum<<< 1,numCircles*numBlocks/2 >>>(circleBlockArray);
   gpuErrchk(cudaDeviceSynchronize());
   int* num = (int*) malloc(sizeof(int));
   cudaMemcpy(num,circleBlockArray,sizeof(int),cudaMemcpyDeviceToHost);
 ...
 //  thrust::device_ptr<int> t_circlePerBlock(circlePerBlock);
  // thrust::device_vector<int> d_circlePerBlock(t_circlePerBlock, t_circlePerBlock + numBlocks+1);
   ...
   for(int i=0; i < numBlocks; i++){
     if(i==0){
         ...
         auto num = fillPairs(d_circleBlockArray.get()+(i*numCircles),numCircles,d_circleBlockIdx.get()+host_circlePerBlock[i]);
  ...
  // cudaMemcpy(host_circlePerBlock,circlePerBlock,sizeof(int)*(numBlocks+1),cudaMemcpyDeviceToHost);
    ...
    for(int i=0; i<numBlocks+1;i++){
       printf(" box[%d][%d]= %d \n",(i/blocksPerRow),(i%blocksPerRow),host_circlePerBlock[i]);
