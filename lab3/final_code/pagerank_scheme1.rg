...
task update_delta(r_pages_src : region(Page),
                  ...
                  num_pages : uint64)
where
    reads writes(r_pages_src.rank),
    writes(r_pages_src.old_rank, r_pages_src.delta),
    reads(r_pages_src.num_out_edges)
do
...
task toplevel()
  ...
  var error_region_part = partition(equal, error_region, ispace(int1d, config.parallelism))
  ...
  var dst_partition = partition(equal, r_pages, colors)

  -- partition links - source partition
  var link_partition = preimage(r_links, dst_partition, r_links.dst_page)

  -- get destination nodes
  var src_partition = image(r_pages, link_partition, r_links.src_page)
  ...
  while not converged do
    ...
    for c in dst_partition.colors do
        update_delta (dst_partition[c], config.damp, config.num_pages)
    end
    ...
    for part in error_region_part.colors do
       for error_val in error_region_part[part] do
            error_val.error = update_diff(dst_partition[part])
        end
      --c.printf("l2_norm = %f\n", l2_norm)
    end
