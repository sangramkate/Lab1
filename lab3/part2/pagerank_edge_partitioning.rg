...
task calculate_delta(r_src_pages: region(Page), 
                      damp      : double)
where 
  reads (r_src_pages.num_outgoing_edges, r_src_pages.rank),
  reads writes (r_src_pages.delta) -- TODO: convert to writes 
do
...
-- TODO: this seems like a hack : how to actually create partitions ? 
  -- var pages_dest_part_disjoint = image(disjoint, complete, r_pages, links_part, r_links.dest_page)
  var pages_dest_part_disjoint = partition(equal, r_pages, ispace(int1d, num_pieces))
-- TODO: is the number of src and dest partitions same? -- and is it same as # of link partitions  --- or do I need to use union?
 ...
 --       c.printf("========\n")
 --       for page in pages_dest_part_disjoint[part] do
 --           c.printf("dest: %d\n", page)
 --       end
 --   end
  ...
  while not converged do
   ...
   -- Rank is updated only for destination pages -- keeping it disjoint
    for part in pages_dest_part_disjoint.colors do
      init_rank(pages_dest_part_disjoint[part], config.damp, config.num_pages)
    end
    ...
    for part in pages_dest_part_disjoint.colors do
      l2_norm += update_diff(pages_dest_part_disjoint[part])
      --c.printf("l2_norm = %f\n", l2_norm)
    end
