...
saxpyCuda(long total_elems, float alpha, float* xarray, float* yarray, float* resultarray, int partitions) {
    ...
    //TODO: How do we time in streams? NVPROF is the solution right? 
    //Arrays for the stream
    ...
    for (int i=0; i<partitions; i++) {
        ...
        part_size = (total_elems/partitions);
        if(i == (partitions - 1)) {
            part_size = total_elems - ((partitions - 1)*(total_elems/partitions));
        }
        //printf("part size : %ld\n", part_size);
        ...
        //Copy to GPU
         gpuErrchk(cudaMemcpyAsync((device_x + (i*part_size)), (xarray + (i*part_size)), part_size*sizeof(float), cudaMemcpyHostToDevice, streams[i]));
         gpuErrchk(cudaMemcpyAsync((device_y + (i*part_size)), (yarray + (i*part_size)), part_size*sizeof(float), cudaMemcpyHostToDevice, streams[i]));
        ...
        saxpy_kernel<<<(part_size + threadsPerBlock - 1)/threadsPerBlock, threadsPerBlock, 0, streams[i]>>>(total_elems, alpha, device_x + i*part_size, device_y + i*part_size, device_result + i*part_size);
        ...
        gpuErrchk(cudaMemcpyAsync((resultarray + (i*part_size)), (device_result + (i*part_size)), part_size*sizeof(float), cudaMemcpyDeviceToHost, streams[i]));
        
        double endMemCpyToHost = CycleTimer::currentSeconds();
        timeCopyD2HAvg += (endMemCpyToHost - startMemCpyToHost);
    }
