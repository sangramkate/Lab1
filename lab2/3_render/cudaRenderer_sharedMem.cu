...
__global__ void kernelRenderCircles(int imageWidth, int imageHeight) {
    ...
    for (int tid_x = threadIdx.x ; tid_x < (imageWidth/gridDim.x); tid_x +=blockDim.x) {
        for(int tid_y = threadIdx.y ; tid_y < (imageHeight/gridDim.y); tid_y +=blockDim.y) { 
            ...
            for (int k = 0 ; k < 4; k ++) {
                shared_imgData[4 * (tid_y*(imageWidth/gridDim.x) + tid_x) + k] = cuConstRendererParams.imageData[(4 * (y * imageWidth + x)) + k];
            }
        }
    }
    ...
    //    const unsigned int offset = blockIdx.x*blockDim.x + threadIdx.x;
        ...
        if(threadIdx.x + threadIdx.y == 0) {
            circleInBox = circleInBoxConservative(p.x, p.y, rad, 
        ...
        for(int tid_x = threadIdx.x ; tid_x < (imageWidth/gridDim.x); tid_x +=blockDim.x) {
            for(int tid_y = threadIdx.y ; tid_y < (imageHeight/gridDim.y); tid_y +=blockDim.y) {
                float4* shared_imgPtr = (float4*)(&shared_imgData[4 * (tid_y * (imageWidth/gridDim.x) + tid_x)]);
                int x = blockIdx.x*(imageWidth/gridDim.x) + tid_x;
                int y = blockIdx.y*(imageHeight/gridDim.y) + tid_y;
     ...
     for(int tid_x = threadIdx.x ; tid_x < (imageWidth/gridDim.x); tid_x +=blockDim.x) {
         for(int tid_y = threadIdx.y ; tid_y < (imageHeight/gridDim.y); tid_y +=blockDim.y) {
             ...
             float r = shared_imgData[4 * (tid_y*(imageWidth/gridDim.x) + tid_x)];
             float g = shared_imgData[4 * (tid_y*(imageWidth/gridDim.x) + tid_x) + 1];
             float b = shared_imgData[4 * (tid_y*(imageWidth/gridDim.x) + tid_x) + 2];
             float a = shared_imgData[4 * (tid_y*(imageWidth/gridDim.x) + tid_x) + 3];
